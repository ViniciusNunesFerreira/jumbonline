<div>
    <form x-data="{ dirty: new Set() }" x-on:variant-pricing-updated.window="dirty.clear()" wire:submit.prevent="save">
        <x-card class="relative overflow-hidden">
            ...
            <x-slot:content>
                <fieldset wire:target="save" wire:loading.delay.attr="disabled" class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div class="col-span-1">
                        ...
                        />
                        <x-input-error for="variant.price" class="mt-2" />
                    </div>
                    <div class="col-span-1">
                        <x-input-label for="variantComparePriceInput" :value="__('Comparar Preço')" />
                        <div class="mt-1 relative">
                            ...
                            <div class="absolute inset-y-0 right-0 pr-3 flex items-center">
                                <x-heroicon-s-question-mark-circle
                                    ...
                                    data-tippy-content="{{ __('Para mostrar um preço reduzido, mova o preço da variante original para Comparar pelo preço. Insira um valor mais baixo em Preço.') }}"
                                    data-tippy-placement="bottom"
                                />
                            </div>
                        </div>
                        <x-input-error for="variant.compare_price" class="mt-2" />
                    </div>
                </fieldset>

                <hr class="my-6 border-slate-100 dark:border-white/5">

                <fieldset wire:target="save" wire:loading.attr="disabled" class="grid grid-cols-1 sm:grid-cols-2 gap-6 items-center">
                    <div @class(['col-span-1 sm:col-span-2' => $variant->cost_price <= 0])>
                        ...
                        <x-input-money
                            ...
                            class="mt-1 block w-full rounded-xl sm:text-sm"
                        />
                        <x-input-error for="variant.cost_price" class="mt-2" />
                        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                            {{ __('Os clientes não verão isso') }}
                        </p>
                    </div>
                    @if($variant->cost_price > 0)
                        <dl class="grid grid-cols-2 gap-6">
                            <div>
                                <dt class="text-sm font-medium text-slate-500 dark:text-slate-400">{{ __('Margem') }}</dt>
                                <dd class="mt-1 text-sm font-semibold text-primary dark:text-slate-200">
                                    {{ $variant->profit_margin }}%
                                </dd>
                            </div>
                            <div>
                                <dt class="text-sm font-medium text-slate-500 dark:text-slate-400">{{ __('Lucro') }}</dt>
                                <dd class="mt-1 text-sm font-semibold text-primary dark:text-slate-200">
                                    <x-money :amount="$variant->gross_profit" :currency="config('app.currency')" />
                                </dd>
                            </div>
                        </dl>
                    @endif

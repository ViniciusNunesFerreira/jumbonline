    ...
    <x-card class="overflow-hidden">
        <x-slot:header>
            <div class="flex items-center justify-between flex-wrap gap-3 sm:flex-nowrap">
                <h3 class="text-base font-semibold text-primary dark:text-slate-200">
                    {{ __('Grupos de Produto') }}
                </h3>
                @if(count($prison->collections))
        ...
        <x-slot:content class="-mx-4 -my-5 sm:-mx-6">
            <ul class="divide-y divide-slate-100 dark:divide-white/5">
                @forelse($prison->collections as $product)
                    <li class="flex items-center justify-between px-4 py-3.5 sm:px-6 transition-colors hover:bg-slate-50/80 dark:hover:bg-white/[0.03]">
                        ...
                        <a  href="{{ route('employee.collections.detail', $product) }}"
                            class="truncate font-medium text-sm text-primary hover:text-accent-600 dark:text-slate-200 dark:hover:text-accent-400"
                        >
                            {{ $product->title }}
                        </a>
                        <button wire:click.prevent="delete('{{ $product->slug }}')" type="button" class="text-slate-400 hover:text-red-500 flex-shrink-0">
                            <span class="sr-only">{{ __('Remover grupo') }}</span>
                ...
                @empty
                    <li class="py-8 text-center">
                        ...
                        <h3 class="mt-2 text-sm font-semibold text-primary dark:text-slate-200">
                            {{ __('Sem grupos de produto') }}
                        </h3>
                        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                            {{ __('Não há grupos vinculados') }}
                        </p>
                        <div class="mt-4">
